...
All tests that need a redirected LOGS_DIR/SESSIONS_DIR use the shared
tmp_zeppeli_dirs fixture (tests/conftest.py), which always redirects both.
"""
...
def test_log_path_uses_full_session_id(tmp_zeppeli_dirs):
...
def test_log_session_started_shape(tmp_zeppeli_dirs):
    sid = "sess-1"
...
def test_log_session_started_reasoning_mode_defaults_to_unavailable(tmp_zeppeli_dirs):
    sid = "sess-1b"
...
def test_log_run_started_shape(tmp_zeppeli_dirs):
    sid = "sess-2"
...
def test_log_run_started_preview_is_truncated(tmp_zeppeli_dirs):
    sid = "sess-3"
    long_prompt = "x" * 500
...
def test_log_model_activity_shape(tmp_zeppeli_dirs):
    sid = "sess-4"
...
def test_log_run_completed_shape_completed(tmp_zeppeli_dirs):
    sid = "sess-5"
...
def test_log_run_completed_shape_failed_includes_error(tmp_zeppeli_dirs):
    sid = "sess-6"
...
def test_log_cli_error_shape(tmp_zeppeli_dirs):
    sid = "sess-7"
...
def test_log_cli_error_message_is_capped(tmp_zeppeli_dirs):
    sid = "sess-8"
...
def test_flush_pending_events_on_empty_queue_returns_immediately(tmp_zeppeli_dirs):
    t0 = time.monotonic()
    flush_pending_events()
    assert time.monotonic() - t0 < 0.1


def test_log_does_not_block_caller(tmp_zeppeli_dirs, monkeypatch):
    ...
    monkeypatch.setattr(eventlog, "_append_jsonl", _slow_append)
    t0 = time.monotonic()
...
def test_log_processes_writes_in_fifo_order(tmp_zeppeli_dirs):
    sid = "sess-fifo"
    for i in range(5):
...
def test_writer_thread_survives_a_failed_write(tmp_zeppeli_dirs, monkeypatch):
    ...
    monkeypatch.setattr(eventlog, "_append_jsonl", _fail_once_then_succeed)
...
def test_stream_response_emits_model_activity_when_ids_given(tmp_zeppeli_dirs):
...
def test_stream_response_omits_model_activity_without_full_identity(tmp_zeppeli_dirs):
    console = Console(file=io.StringIO())
    live = SimpleLive(console)
...
def test_main_logs_cli_error_on_uncaught_exception(tmp_zeppeli_dirs, monkeypatch):
    ...
    monkeypatch.setattr(repl, "load_llm", _boom)
