"""Automated tests for permission-mode logic — no Ollama/LLM dependency, no
real TTY interaction. Safe to run anytime (e.g. with Ollama stopped) and
exits non-zero on failure, unlike test_tool_call.py (which needs a live
model and is inspected by eye).

Covers:
- build_pre_tool_hooks() dispatch for all three modes
- _is_within_cwd() path-containment edge cases
- the auto-mode hook's inside/outside-cwd branching
- cli.py's argparse: --yolo-mode/--auto-mode mutual exclusivity, -p/--prompt
- confirm_auto_mode_trust()'s _arrow_menu-index -> bool mapping
- ui.repl.main()'s auto-mode trust gate: declining skips load_llm(), and
  one-shot -p mode never triggers the trust check at all
- ui.repl._clear_input: Esc clears the live input buffer, no-ops if empty
- ui.repl main()'s per-process session ID: uniqueness, and its rendering in
  _get_toolbar()

Tests that call ui.repl.main() for real also redirect
core.sessions.SESSIONS_DIR and core.eventlog.LOGS_DIR to a temp dir via the
shared tmp_zeppeli_dirs fixture (tests/conftest.py) (main() now
unconditionally writes a session-history file and an event-log file — see
test_sessions.py / test_eventlog.py for those features' own coverage) so
this suite never touches the developer's real ~/.zeppeli/sessions/ or
~/.zeppeli/logs/.
"""

import asyncio
import uuid
from pathlib import Path
from types import SimpleNamespace

import pytest
from prompt_toolkit.buffer import Buffer
from rich.console import Console

import cli
import ui.permissions as permissions
import ui.repl as repl
from ui.permissions import (
    MODE_APPROVAL,
    MODE_AUTO,
    MODE_YOLO,
    _is_within_cwd,
    _make_auto_hook,
    build_pre_tool_hooks,
    confirm_auto_mode_trust,
)

_console = Console()


@pytest.fixture(autouse=True)
def _reset_approval_state():
    permissions._turn_approved.clear()
    permissions._session_approved.clear()
    yield
    permissions._turn_approved.clear()
    permissions._session_approved.clear()


def test_build_pre_tool_hooks_yolo():
    assert build_pre_tool_hooks(MODE_YOLO, "/any/cwd") == {}


def test_build_pre_tool_hooks_approval_shape():
    # permission_ask() now needs a `live` argument only available at call
    # time (ui/live_region.py's LiveRegion/SimpleLive), so build_pre_tool_
    # hooks() binds a fresh closure per call instead of returning
    # permission_ask itself — check shape/behavior, not identity.
    hooks = build_pre_tool_hooks(MODE_APPROVAL, "/any/cwd")
    assert set(hooks.keys()) == {"write_file", "delete_file"}
    assert callable(hooks["write_file"]) and callable(hooks["delete_file"])
    assert hooks["write_file"] is hooks["delete_file"]
    assert hooks is not permissions.PRE_TOOL_HOOKS


def test_build_pre_tool_hooks_approval_delegates_to_permission_ask(monkeypatch):
    async def _fake(tool_name, args, console, live):
        return tool_name == "write_file"

    monkeypatch.setattr(permissions, "permission_ask", _fake)
    hooks = build_pre_tool_hooks(MODE_APPROVAL, "/any/cwd")
    assert asyncio.run(hooks["write_file"]("write_file", {"path": "x"}, _console)) is True
    assert asyncio.run(hooks["delete_file"]("delete_file", {"path": "x"}, _console)) is False


def test_build_pre_tool_hooks_auto_shape():
    hooks = build_pre_tool_hooks(MODE_AUTO, "/any/cwd")
    assert set(hooks.keys()) == {"write_file", "delete_file"}
    assert callable(hooks["write_file"]) and callable(hooks["delete_file"])
    assert hooks["write_file"] is hooks["delete_file"]


def test_is_within_cwd_relative_inside(tmp_path):
    assert _is_within_cwd(str(tmp_path / "child" / "file.txt"), str(tmp_path)) is True


def test_is_within_cwd_absolute_inside(tmp_path):
    assert _is_within_cwd(str(tmp_path / "file.txt"), str(tmp_path)) is True


def test_is_within_cwd_absolute_outside(tmp_path):
    assert _is_within_cwd("/etc/passwd", str(tmp_path)) is False


def test_is_within_cwd_dotdot_escape_outside(tmp_path):
    escaped = str(tmp_path / ".." / "escape.txt")
    assert _is_within_cwd(escaped, str(tmp_path)) is False


def test_auto_hook_inside_cwd_no_prompt(tmp_path, monkeypatch):
    async def _spy(*a, **k):
        raise AssertionError("permission_ask should not be called inside cwd")

    monkeypatch.setattr(permissions, "permission_ask", _spy)
    hook = _make_auto_hook(str(tmp_path), live=None)
    path = str(tmp_path / "f.txt")
    assert asyncio.run(hook("write_file", {"path": path}, _console)) is True


def test_auto_hook_outside_cwd_delegates_true(tmp_path, monkeypatch):
    calls = []

    async def _fake(*a, **k):
        calls.append(a)
        return True

    monkeypatch.setattr(permissions, "permission_ask", _fake)
    hook = _make_auto_hook(str(tmp_path), live=None)
    assert asyncio.run(hook("write_file", {"path": "/etc/passwd"}, _console)) is True
    assert len(calls) == 1


def test_auto_hook_outside_cwd_delegates_false(tmp_path, monkeypatch):
    async def _fake(*a, **k):
        return False

    monkeypatch.setattr(permissions, "permission_ask", _fake)
    hook = _make_auto_hook(str(tmp_path), live=None)
    assert asyncio.run(hook("delete_file", {"path": "/etc/passwd"}, _console)) is False


def test_cli_mutex_yolo_and_auto_raises_systemexit_2():
    try:
        cli._parse_args(["--yolo-mode", "--auto-mode"])
        assert False, "expected SystemExit"
    except SystemExit as e:
        assert e.code == 2


def test_cli_prompt_flag_short_and_long():
    assert cli._parse_args(["-p", "hi"]).prompt == "hi"
    assert cli._parse_args(["--prompt", "hi"]).prompt == "hi"


def test_cli_prompt_default_none():
    assert cli._parse_args([]).prompt is None


def test_cli_prompt_combines_with_mode_flags():
    ns = cli._parse_args(["--yolo-mode", "-p", "hi"])
    assert ns.yolo_mode is True
    assert ns.prompt == "hi"


def test_confirm_auto_mode_trust_yes(monkeypatch):
    monkeypatch.setattr(permissions, "_arrow_menu", lambda *a, **k: 0)  # "Yes, I trust this folder"
    assert confirm_auto_mode_trust(_console) is True


def test_confirm_auto_mode_trust_no(monkeypatch):
    monkeypatch.setattr(permissions, "_arrow_menu", lambda *a, **k: 1)  # "No, exit"
    assert confirm_auto_mode_trust(_console) is False


def test_confirm_auto_mode_trust_cancelled(monkeypatch):
    monkeypatch.setattr(permissions, "_arrow_menu", lambda *a, **k: None)  # Ctrl+C
    assert confirm_auto_mode_trust(_console) is False


def test_main_auto_mode_declined_skips_llm_load(monkeypatch):
    def _spy(*a, **k):
        raise AssertionError("load_llm should not be called when trust is declined")

    monkeypatch.setattr(repl, "confirm_auto_mode_trust", lambda console: False)
    monkeypatch.setattr(repl, "load_llm", _spy)
    repl.main(mode=MODE_AUTO, prompt=None)  # should return early, no exception


def test_main_prompt_mode_skips_trust_check(tmp_zeppeli_dirs, monkeypatch):
    def _spy(console):
        raise AssertionError("confirm_auto_mode_trust should not be called in -p mode")

    async def _fake_run_turn(*a, **k):
        return None

    monkeypatch.setattr(repl, "confirm_auto_mode_trust", _spy)
    monkeypatch.setattr(repl, "load_llm", lambda **k: object())
    monkeypatch.setattr(repl, "run_turn", _fake_run_turn)
    monkeypatch.setattr(repl, "model_supports_reasoning", lambda *a, **k: False)  # no real Ollama call
    monkeypatch.setattr(repl, "get_context_window", lambda *a, **k: None)  # no real Ollama call
    repl.main(mode=MODE_AUTO, prompt="hi")


def test_clear_input_esc_empties_nonempty_buffer():
    buffer = Buffer()
    buffer.text = "hello world"
    event = SimpleNamespace(app=SimpleNamespace(current_buffer=buffer))
    repl._clear_input(event)
    assert buffer.text == ""


def test_clear_input_esc_noop_on_empty_buffer():
    buffer = Buffer()
    event = SimpleNamespace(app=SimpleNamespace(current_buffer=buffer))
    repl._clear_input(event)  # should not raise
    assert buffer.text == ""


def test_main_seeds_unique_session_id(tmp_zeppeli_dirs, monkeypatch):
    async def _fake_run_turn(*a, **k):
        return None

    monkeypatch.setattr(repl, "load_llm", lambda **k: object())
    monkeypatch.setattr(repl, "run_turn", _fake_run_turn)
    monkeypatch.setattr(repl, "model_supports_reasoning", lambda *a, **k: False)  # no real Ollama call
    monkeypatch.setattr(repl, "get_context_window", lambda *a, **k: None)  # no real Ollama call

    repl.main(mode=MODE_APPROVAL, prompt="hi")
    first_id = repl._session_state["id"]
    repl.main(mode=MODE_APPROVAL, prompt="hi")
    second_id = repl._session_state["id"]

    uuid.UUID(first_id)  # raises ValueError if not a valid UUID string
    uuid.UUID(second_id)
    assert first_id != second_id


def test_get_toolbar_includes_session_id_line():
    fixed_id = str(uuid.uuid4())
    repl._session_state["id"] = fixed_id
    tokens = repl._get_toolbar()
    text = "".join(t for _, t in tokens)

    assert f"Session ID: {fixed_id}" in text
    # 3 + len(SLASH_COMMANDS) *lines* means 2 + len(SLASH_COMMANDS) newline
    # characters joining them (N lines need N-1 separators).
    assert text.count("\n") == 2 + len(repl.SLASH_COMMANDS)
